...
class ActivityMassSeeder extends Seeder
{
    ...
    private function createSystemActivities(): void
    {
        ...
        $this->command->info('✅ Create '.$activities->count()/** @phpstan-ignore method.nonObject */.' attività di sistema');
    }
    ...
    private function createSnapshots(): void
    {
        ...
        // Crea 500 snapshot
        /** @var \Illuminate\Database\Eloquent\Collection<int, Snapshot> $snapshots */
        $snapshots = Snapshot::factory()->count(500)->create([
            'created_at' => Carbon::now()->subDays(rand(1, 180)),
        ]);

        $this->command->info('✅ Creati '.$snapshots->count()/** @phpstan-ignore method.nonObject */.' snapshot');
    }
    ...
    private function createStoredEvents(): void
    {
        ...
        /** @var \Illuminate\Database\Eloquent\Collection<int, StoredEvent> $events */
        $events = StoredEvent::factory()->count(1000)->create([
            'created_at' => Carbon::now()->subDays(rand(1, 365)),
        ]);

        $this->command->info('✅ Creati '.$events->count()/** @phpstan-ignore method.nonObject */.' eventi memorizzati');
    }
